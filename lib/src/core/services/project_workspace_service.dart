...
class ProjectWorkspaceService {
  ...
  Future<void> ensureStarterFiles({
    required String projectId,
    required String framework,
    required String projectName,
    required String description,
  }) async {
    await workspaceDirectory(projectId);
    if (framework.trim().toLowerCase() != 'flutter') return;
    ...
    while (packageName.startsWith('_')) {
      packageName = packageName.substring(1);
    }
    while (packageName.endsWith('_')) {
      packageName = packageName.substring(0, packageName.length - 1);
    }
    if (packageName.isEmpty) packageName = 'nexus_app';

    final cleanDescription = description.trim().isEmpty
        ? 'A Flutter project created with Nexus.'
        : description.trim().replaceAll('\n', ' ');

    Future<void> ensureTextFile(
      String path,
      String fileContent, {
      bool repairBlank = true,
    }) async {
      ...
      if (repairBlank && (await file.readAsString()).trim().isEmpty) {
        await file.writeAsString(fileContent);
      }
    }
...
''',
    ...
    final safeTitle = projectName.replaceAll("'", "\\'");
    await ensureTextFile(
      'lib/main.dart',
      '''import 'package:flutter/material.dart';

void main() {
  runApp(const NexusStarterApp());
}

class NexusStarterApp extends StatelessWidget {
  const NexusStarterApp({super.key});
...
''',
  ...
  Future<String> _resolve(String projectId, String relativePath) async {
    ...
    if (candidate != workspace.path &&
        !p.isWithin(workspace.path, candidate)) {
      throw StateError('Path escapes the Nexus project workspace.');
    }
